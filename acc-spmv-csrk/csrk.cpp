...
#pragma acc enter data copyin(x_test[N])
}

void CSRk_Graph::setY(float *y_vec) {
  y = y_vec;
#pragma acc enter data copyin(y[N])
}

float *CSRk_Graph::getY() {
#pragma acc exit data copyout(y[N])
  return y;
}
...
#pragma omp parallel for schedule(runtime)
  ...
  {
#pragma acc data present(x_test[N], y[N], r_vec[N + 1], c_vec[NNZ], val[NNZ],  \
                         this, mapCoarseToFinerRows_gpu[numCoarsestRows + 1])
    {
#pragma acc region
      {
#pragma acc loop gang
        for (int i_sup_row = 0; i_sup_row < numCoarsestRows; i_sup_row++) {
          ...
          const unsigned int end_index =
              mapCoarseToFinerRows_gpu[i_sup_row + 1];

#pragma acc loop worker(16), vector(16)
...
#pragma acc enter data copyin(val[NNZ], r_vec[N + 1], c_vec[NNZ])
...
#pragma omp for schedule(static) nowait
  ...
  mapCoarseToFinerRows_gpu = new unsigned int[numCoarsestRows + 1];
  memcpy(mapCoarseToFinerRows_gpu, mapCoarseToFinerRows[k - 1],
         sizeof(unsigned int) * (numCoarsestRows + 1));
#pragma acc enter data copyin(this,                                            \
                              mapCoarseToFinerRows_gpu[numCoarsestRows + 1])
