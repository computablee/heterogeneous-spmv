...
int main(int argc, char **argv) {
  if (argc < 4) {
    printf(
        "Syntax: %s inputfile num_runs super_super_row_size super_row_size\n",
        argv[0]);
    return 0;
  }
  int nRows, nCols;
...
#pragma omp for schedule(static)
  ...
  for (int i = 0; i < 5; i++) {
    hipLaunchKernelGGL(cuSpMV_3, dim3(A_mat.getNumCoarsestRows()), dim3(8, 12), 0, 0, numCoarsestRows_gpu, mapCoarseToFinerRows_gpu_outer,
