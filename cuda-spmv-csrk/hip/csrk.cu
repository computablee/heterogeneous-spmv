...
void CSRk_Graph::setX(float *x) {
  ...
  hipMalloc(&x_test_gpu, sizeof(float) * N);
  hipMemcpy(x_test_gpu, x_test, sizeof(float) * N, hipMemcpyHostToDevice);
}

void CSRk_Graph::setY(float *y_vec) {
  y = y_vec;
  hipMalloc(&y_gpu, sizeof(float) * N);
  hipMemcpy(y_gpu, y, sizeof(float) * N, hipMemcpyHostToDevice);
}
...
#pragma omp barrier
  ...
  hipMalloc(&val_gpu, sizeof(float) * NNZ);
  hipMalloc(&r_vec_gpu, sizeof(unsigned int) * (N + 1));
  hipMalloc(&c_vec_gpu, sizeof(unsigned int) * NNZ);
  hipMemcpy(val_gpu, val, sizeof(float) * NNZ, hipMemcpyHostToDevice);
  hipMemcpy(r_vec_gpu, r_vec, sizeof(unsigned int) * (N + 1),
             hipMemcpyHostToDevice);
  hipMemcpy(c_vec_gpu, c_vec, sizeof(unsigned int) * NNZ,
             hipMemcpyHostToDevice);
...
#pragma omp for schedule(static) nowait
  ...
  hipMalloc(&mapCoarseToFinerRows_gpu_outer,
             sizeof(unsigned int) * (numCoarsestRows + 1));
  hipMemcpy(mapCoarseToFinerRows_gpu_outer, mapCoarseToFinerRows[k - 1],
             sizeof(unsigned int) * (numCoarsestRows + 1),
             hipMemcpyHostToDevice);
  hipMalloc(&mapCoarseToFinerRows_gpu_inner,
             sizeof(unsigned int) * (numCoarsishRows + 1));
  hipMemcpy(mapCoarseToFinerRows_gpu_inner, mapCoarseToFinerRows[k - 2],
             sizeof(unsigned int) * (numCoarsishRows + 1),
             hipMemcpyHostToDevice);
  hipMalloc(&numCoarsestRows_gpu, sizeof(unsigned int));
  hipMalloc(&N_gpu, sizeof(unsigned int));
  hipMalloc(&M_gpu, sizeof(unsigned int));
  hipMalloc(&NNZ_gpu, sizeof(unsigned int));
  hipMemcpy(N_gpu, &N, sizeof(unsigned int), hipMemcpyHostToDevice);
  hipMemcpy(M_gpu, &M, sizeof(unsigned int), hipMemcpyHostToDevice);
  hipMemcpy(NNZ_gpu, &NNZ, sizeof(unsigned int), hipMemcpyHostToDevice);
  hipMemcpy(numCoarsestRows_gpu, &numCoarsestRows, sizeof(unsigned int),
             hipMemcpyHostToDevice);
