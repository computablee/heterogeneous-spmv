...
int main(int argc, char **argv) {
  ...
  int ssrs = (int)floor(0.5 + (8.489 - 1.15 * log(d)));
  int srs = (int)floor(0.5 + (10.711 - 1.607 * log(d)));
  bool vec = false;
  int veclevel = 4;
...
#pragma omp for schedule(static)
  ...
  for (int i = 0; i < 5; i++) {
    if (vec)
      hipLaunchKernelGGL(cuSpMV_3_vec, dim3(A_mat.getNumCoarsestRows()), dim3(veclevel, blockDimx, blockDimy), 0, 0, numCoarsestRows_gpu, mapCoarseToFinerRows_gpu_outer,
          mapCoarseToFinerRows_gpu_inner, r_vec_gpu, c_vec_gpu, val_gpu,
          x_test_gpu, y_gpu);
    else
      hipLaunchKernelGGL(cuSpMV_3, dim3(A_mat.getNumCoarsestRows()), dim3(blockDimx, blockDimy), 0, 0, numCoarsestRows_gpu, mapCoarseToFinerRows_gpu_outer,
          mapCoarseToFinerRows_gpu_inner, r_vec_gpu, c_vec_gpu, val_gpu,
          x_test_gpu, y_gpu);
