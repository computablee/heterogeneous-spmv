...
void acc_spmv(int m, int n, int nnz, int *restrict row_start,
              int *restrict col_idx, float *restrict val, float *restrict x,
              float *restrict y) {

  int row, col, val_idx;
#pragma acc data present(row_start [0:m + 1], col_idx [0:nnz], val [0:nnz],    \
                         x [0:m], y [0:m])
  {
#pragma acc region
    {
...
int main(int argc, char *argv[]) {
  ...
  // int N = atoi(argv[2]);
  // int i ;
#pragma acc data copyin(row_ptr [0:m + 1], col_idx [0:nnz], val [0:nnz],       \
                        x [0:m], y [0:m]) copyout(y [0:m])
  {
    for (i = 0; i < N; ++i) {

      double tic = clock_time();
      acc_spmv(m, n, nnz, row_ptr, col_idx, val, x, y);
