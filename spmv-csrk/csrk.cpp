...
CSRk_Graph::CSRk_Graph(long nRows, long nCols, long nnz, unsigned int *rVec,
  ...
  posix_memalign((void **)&r_vec, 64, (N + 1) * sizeof(unsigned int));
  ...
  posix_memalign((void **)&c_vec, 64, NNZ * sizeof(unsigned int));
  ...
  posix_memalign((void **)&val, 64, NNZ * sizeof(float));
...
#pragma omp barrier
  ...
  posix_memalign((void **)&x_test, 64, N * sizeof(float));
  // x_test = (float *) hbw_malloc(N*sizeof(float));

  if (kernelType == "SpMV" && k > 1) {
    permBigG = new unsigned int[nRows];

    posix_memalign((void **)&mapCoarseToFinerRows, 64,
                   (k + 1) * sizeof(unsigned int *));
    ...
    posix_memalign((void **)&supRowSizes, 64, (k - 1) * sizeof(unsigned int));
  ...
  if (mapCoarseToFinerRows != NULL) {
    for (int i = 0; i < k; i++) {
      if (mapCoarseToFinerRows[i] != NULL)
        hbw_free(mapCoarseToFinerRows[i]);
    }

    hbw_free(mapCoarseToFinerRows);
...
void BAND_k::handCoarsen(int level, int super_node_nnz, CSRk_Graph &csrkGraph) {
  ...
  unsigned int *r_start_coarsened;
  posix_memalign((void **)&r_start_coarsened, 64,
                 (num_coarse_vtxs + 1) * sizeof(unsigned int));
